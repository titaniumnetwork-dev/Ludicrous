...
class Corrosion extends require('../rewrite') {
    ...
    get script() {
        return fs.existsSync(path.join('./corrosion/lib/server', 'bundle.js')) ? fs.readFileSync(path.join('./corrosion/lib/server', 'bundle.js')) : 'Client script is still compiling or has crashed.'
    };
};
